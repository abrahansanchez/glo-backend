...
/**
 * =====================================================
 * G.711 μ-law <-> PCM16 Audio Utilities
 * Used for Twilio Media Streams (8kHz, mono)
 * =====================================================
 */
...
export function mulawToPCM16(base64Payload) {
  try {
    ...
    if (mulawBytes.length === 0) return null;
...
export function pcm16ToMulaw(pcm16Buffer) {
  try {
    if (
      !Buffer.isBuffer(pcm16Buffer) ||
      pcm16Buffer.length === 0 ||
      pcm16Buffer.length % 2 !== 0
    ) {
      return null;
    }
