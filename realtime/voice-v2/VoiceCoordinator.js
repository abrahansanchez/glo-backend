...
export class VoiceCoordinator {
  ...
  handleTimeout(session, timeoutType, context = {}) { const timeout = session.watchdog.trigger(timeoutType, { proposalVersion: session.proposal.proposalVersion, ...context }); if (timeout.responseId) session.responseRegistry.invalidate(timeout.responseId, timeoutType); if (timeout.markId) session.playbackRegistry.stale(timeout.markId, timeoutType); if (timeout.responseId && timeout.markId) { session.confirmationAuthority.revoke({ proposalVersion: timeout.proposalVersion, responseId: timeout.responseId, markId: timeout.markId, reason: timeoutType }); session.record("CONFIRMATION_REVOKED", { proposalVersion: timeout.proposalVersion, responseId: timeout.responseId, markId: timeout.markId, reason: timeoutType }); } session.record("TIMEOUT_RECOVERY_PLANNED", { timeoutType, recoveryPurpose: "ERROR_RECOVERY", proposalVersion: session.proposal.proposalVersion, responseId: timeout.responseId, markId: timeout.markId }); return Object.freeze({ timeout, responsePlan: this.responsePlanner({ proposal: session.proposal, purpose: "ERROR_RECOVERY" }) }); }
