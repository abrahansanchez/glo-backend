...
for (const scenario of [
  ...
  assert.equal(f.app.session, originalSession); assert.equal(f.app.session.businessContext, originalBusiness); assert.equal(f.app.session.proposal.proposalVersion, version); assert.equal("language" in f.app.session.proposal, false); assert.equal(f.app.session.ambiguityRecovery.snapshot.consecutiveAmbiguousTurns, 0);
