...
test("directed confirmation recovery traverses generation, validation, playback, mark, and fresh authority", async () => {
  ...
  f.openai.receive({ type: "response.output_audio_transcript.done", response_id: "fresh-confirm", transcript: JSON.parse(create.response.instructions).speechContract.requiredMessage }); f.openai.receive({ type: "response.done", response: { id: "fresh-confirm", status: "completed" } }); await settle(f.app);
