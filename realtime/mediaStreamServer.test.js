...
const createProductionSession = ({ bookAppointment } = {}) => {
...
test("max_output_tokens waits for playback and creates one 320-token retry", async () => {
  const harness = createConfirmationLifecycleTestHarness({ bookingState: baseBookingState() });
  const mark = await setupDeliveredResponse(harness);
  ...
  assert.equal(retry.payload.response.max_output_tokens, 320);
  ...
  assert.equal(retryRecord.payload.response.max_output_tokens, 320);
...
test("completed but invalidated delivery performs one bounded 320-token retry", async () => {
  const harness = createConfirmationLifecycleTestHarness({ bookingState: baseBookingState() });
...
for (const queued of [false, true]) {
  for (const language of ["en", "es"]) {
    test(`${queued ? "queued" : "immediate"} ${language} confirmation construction uses purpose and 320`, () => {
      const harness = createConfirmationLifecycleTestHarness({ bookingState: baseBookingState() });
      ...
      assert.equal(actual.payload.response.max_output_tokens, 320);
      assert.equal(
...
test("alternative remains 80 and ordinary deterministic response remains 120", () => {
  ...
  assert.equal(alternative.payload.response.max_output_tokens, 80);
  assert.equal(ordinary.payload.response.max_output_tokens, 120);
});

test("final backend-confirmed success construction retains the existing 160-token exact-response behavior", () => {
  const finalSuccess = buildExactResponseRequest(
    'Say this exactly, with no extra words: "Your appointment is confirmed."'
  );
  assert.equal(finalSuccess.response.max_output_tokens, 160);
...
test("production media-send failure invalidates confirmation and permits only one retry", async () => {
  const { ai, twilio, controls } = createProductionSession();
  ...
  assert.equal(record.lifecycleActionHandled, true);
  assert.equal(state.pendingAssistantResponse?.retryCount, 1);
});

test("production mark-send failure invalidates confirmation and permits only one retry", async () => {
  const { ai, twilio, controls } = createProductionSession();
...
test("production watchdog ownership survives cleanup and WebSocket close", async () => {
  const { ai, twilio, controls } = createProductionSession();
  ...
  assert.equal(controls.getState().activeConfirmationLifecycleId, "resp-watchdog-prod");
  twilio.readyState = 3;
...
for (const language of ["en", "es"]) {
  test(`production immediate ${language} confirmation dispatch uses 320`, async () => {
    const { ai, controls } = createProductionSession();
    controls.seedBookingState({
      state: baseBookingState(),
    ...
    assert.equal(sent.response.max_output_tokens, 320);
  });

  test(`production ${language} confirmation queue insertion and flush preserves purpose and 320`, async () => {
    const { ai, controls } = createProductionSession();
    controls.seedBookingState({
      state: baseBookingState(),
      availability: { slotChecked: true, slotAvailable: true },
    ...
    assert.equal(queued.maxOutputTokens, 320);
...
test("production stale resp-1 terminal events cannot disturb active retry resp-2", async () => {
  ...
  assert.equal(finalState.pendingAssistantResponse?.reason, "confirmation_retry_failed_recovery");
  assert.notEqual(
